...
![アプリケーション完成図](https://static.us-east-1.prod.workshops.aws/public/180149b9-f35e-46f7-93a7-bfdcb5fec962/static/01-appimage.png)
---

本ハンズオンでは下記の環境を構築します。

![アーキテクチャ図](https://static.us-east-1.prod.workshops.aws/public/180149b9-f35e-46f7-93a7-bfdcb5fec962/static/01-01-architecture.png)
...
# 3. AWS Amplify への HTML ファイルアップロード
...
![マネジメントコンソールでサービス名検索](https://static.us-east-1.prod.workshops.aws/public/180149b9-f35e-46f7-93a7-bfdcb5fec962/static/04-Amplify01.png)
...
2. アプリケーションの作成画面に遷移します。アプリケーションのデプロイ方法を選択する画面に移るので、「**Gitなしでデプロイ**」を選択し、「**次へ**」を押します。  
    ![作成方法の選択](https://static.us-east-1.prod.workshops.aws/public/180149b9-f35e-46f7-93a7-bfdcb5fec962/static/04-Amplify03.png)
...
3. 手動デプロイ開始画面に遷移するので、アプリケーションの名前を「`SimpleBedrock`」と入力してブランチ名に「`main`」と入力します。そして、フロントエンドファイルとして作成した「`index.html`」ファイルをWindowsエクスプローラーやFinder上でZipファイル「`index.html.zip`」として圧縮して、画面下部のファイルアップロード領域にドラッグ&ドロップします。準備ができたら、「**保存してデプロイ**」ボタンを押します。  
    ![手動デプロイ](https://static.us-east-1.prod.workshops.aws/public/180149b9-f35e-46f7-93a7-bfdcb5fec962/static/04-Amplify04.png)

4. デプロイ画面が表示されるのでしばらく待ちます。  
    ![デプロイ](https://static.us-east-1.prod.workshops.aws/public/180149b9-f35e-46f7-93a7-bfdcb5fec962/static/04-Amplify05.png)

5. デプロイが完了するとURLが表示されますので、「**デプロイされたURLにアクセス**」ボタンを押し、HTML ファイルにアクセスできることを確認してください。確認できたら次のステップに進みます。  
    ![バケットのアクセス許可](https://static.us-east-1.prod.workshops.aws/public/180149b9-f35e-46f7-93a7-bfdcb5fec962/static/04-Amplify06.png)
...
# 5. Lambda関数の作成
このセクションでは、Amazon Bedrock を呼び出す Lambda 関数を作成します。以下にあるサンプルコードは Claude Sonnet 4.5 を呼び出すシンプルな Python プログラムです。 AWS Lambdaにて動作させるファイルは以下のとおりです。
...
このセクションでは、Amazon BedrockのConverse APIを使ってLambda関数の実行部分で利用するパラメータとレスポンスの形式を定義します。Lambdaに渡されるイベントデータの中にフロントエンドから渡されたユーザー入力メッセージがありますので、それをuser_prompt変数に格納し、user_message配列に入れてユーザープロンプトとします。model_idパラメーターでは利用する基盤モデルを指定します。利用したいモデルに応じて、モデルIDの値を書き換えます。
...
1. マネジメントコンソールから AWS Lambda のコンソール画面を開き、画面右の「**関数を作成**」画面を押します。 ![マネジメントコンソールでサービス名検索](https://static.us-east-1.prod.workshops.aws/public/180149b9-f35e-46f7-93a7-bfdcb5fec962/static/05-Lambda01.png)

2. 「一から作成」メニューで、関数の名前は「`SimpleBedrock`」、ランタイムは「**Python3.13**」にして、「**関数の作成**」ボタンを押します。  
    ![関数の作成](https://static.us-east-1.prod.workshops.aws/public/180149b9-f35e-46f7-93a7-bfdcb5fec962/static/05-Lambda02.png)

3. 関数が作成されますので、コード入力欄の既存のコードを削除し、上記のコード全文をコピー＆ペーストします。  
    ![コードの貼り付け関数の作成](https://static.us-east-1.prod.workshops.aws/public/180149b9-f35e-46f7-93a7-bfdcb5fec962/static/05-Lambda03.png)
　
...
5. Lambda 関数はタイムアウト時間が初期状態だと3秒に設定されており、言語モデルの推論に間に合わないためこの時間を延長します。「設定」タブの「一般設定」メニューの「**編集**」ボタンを押します。  
    ![タイムアウト設定編集](https://static.us-east-1.prod.workshops.aws/public/180149b9-f35e-46f7-93a7-bfdcb5fec962/static/05-Lambda05.png)

6. タイムアウト値を「1分以上」に設定して、「**保存**」ボタンを押します。  
    ![タイムアウト設定編集](https://static.us-east-1.prod.workshops.aws/public/180149b9-f35e-46f7-93a7-bfdcb5fec962/static/05-Lambda06.png)
...
7. また、IAM ロールには Amazon Bedrock を呼び出す権限がまだついていないので付与する必要があります。同じ設定タブの「アクセス権限」を開き、ロール名のリンクをクリックすると IAM ロールの編集画面に遷移しますので、「**許可を追加**」ボタンから「**ポリシーをアタッチ**」を押し、ポリシー追加画面を開きます。ポリシー追加画面では検索ボックスに「Bedrock」と入力して絞り込み、**AmazonBedrockFullAccess**ポリシーにチェックを入れて、「**許可を追加**」ボタンを押します。  
    ![IAMロール編集](https://static.us-east-1.prod.workshops.aws/public/180149b9-f35e-46f7-93a7-bfdcb5fec962/static/05-Lambda07.png)
　 ![IAMロール編集](https://static.us-east-1.prod.workshops.aws/public/180149b9-f35e-46f7-93a7-bfdcb5fec962/static/05-Lambda08.png)

8. ここまで設定できたらプログラム実行の準備が整いました。コード画面に戻り、「Test」ボタンを押すとテストイベントの編集画面が出てきます。「イベント名」に「`Test`」などと入れます。イベントテンプレートは初期状態で出てくる「hello-world」テンプレートを使います。「key1」要素の右にある「`value1`」という文字列を任意の質問で書き換えてください（ダブルクォーテーションを消さないように注意しましょう）。他は残しておいて大丈夫ですので、「**Save**」ボタンを押します。  
    ![テストイベントの編集](https://static.us-east-1.prod.workshops.aws/public/180149b9-f35e-46f7-93a7-bfdcb5fec962/static/05-Lambda09.png)

9. 保存できたら再度「Test」ボタンを押しましょう。テスト結果画面の「Response」に問いかけへの回答が表示されたら成功です！  
    ![テスト実行結果](https://static.us-east-1.prod.workshops.aws/public/180149b9-f35e-46f7-93a7-bfdcb5fec962/static/05-Lambda10.png)
...
3. REST APIの設定をしていきます。  
    **新しいAPI**をクリックし、**API名**に`SimpleBedrockAPI`と入力し、**APIを作成**をクリックします。  
    ![REST APIの設定](https://static.us-east-1.prod.workshops.aws/public/180149b9-f35e-46f7-93a7-bfdcb5fec962/static/06-API03.png)
    
4. APIのメソッドを作成します。**メソッドを作成**をクリックしてください。  
    ![メソッドの作成](https://static.us-east-1.prod.workshops.aws/public/180149b9-f35e-46f7-93a7-bfdcb5fec962/static/06-API04.png)
    
5. **メソッドタイプ**に**POST**を選択し、**統合タイプ**には**Lambda 関数**を選びます。  
    ![メソッドの設定](https://static.us-east-1.prod.workshops.aws/public/180149b9-f35e-46f7-93a7-bfdcb5fec962/static/06-API05.png)
    

6. 下にスクロールし、Lambda 関数を指定します。入力補完が有効なので、`SimpleBedrock`と入力すると一つ前の章で作成したLambda 関数が表示されます。関数名が入力できたら**メソッドを作成**をクリックしてください。  
![メソッドの設定](https://static.us-east-1.prod.workshops.aws/public/180149b9-f35e-46f7-93a7-bfdcb5fec962/static/06-API06.png)

7. このAPIのリソースの設定をしていきます。定義したPOSTメソッドの上にある **/** をクリックし移動します。  
    ![リソースに移動](https://static.us-east-1.prod.workshops.aws/public/180149b9-f35e-46f7-93a7-bfdcb5fec962/static/06-API07.png)
    

8. **CORS を有効にする**をクリックし、CORSの設定をしていきます。  
    ![リソースに移動](https://static.us-east-1.prod.workshops.aws/public/180149b9-f35e-46f7-93a7-bfdcb5fec962/static/06-API08.png)
...
9. **POST**にチェックをいれ、**保存**をクリックします。  
    ![CORS](https://static.us-east-1.prod.workshops.aws/public/180149b9-f35e-46f7-93a7-bfdcb5fec962/static/06-API09.png)
    

10. APIの設定が完了したので、デプロイを行います。**API をデプロイ**をクリックします。 ![APIをデプロイ](https://static.us-east-1.prod.workshops.aws/public/180149b9-f35e-46f7-93a7-bfdcb5fec962/static/06-API10.png)
    
11. **ステージ**に**新しいステージ**を選び、**ステージ名**に`prod`と入力し、**デプロイ**をクリックします。 ![APIをデプロイ](https://static.us-east-1.prod.workshops.aws/public/180149b9-f35e-46f7-93a7-bfdcb5fec962/static/06-API11.png)
    
12. APIのURLが生成されるので、画面中部のURLをコピーして控えます。
    ![APIのURL](https://static.us-east-1.prod.workshops.aws/public/180149b9-f35e-46f7-93a7-bfdcb5fec962/static/06-API12.png)
...
14. 表示されたWebページからBedrockへ質問をしてみましょう。**入力テキスト**に質問を入力し、**送信**をクリックします。  
	回答が返ってきたらハンズオン成功です！  
	![動作確認](https://static.us-east-1.prod.workshops.aws/public/180149b9-f35e-46f7-93a7-bfdcb5fec962/static/06-API20.png)

15. ガードレールがきちんと動作しているか確認するため、あえて不適切な質問をしてみましょう。例：「毒薬の作り方を教えて。」
...
![S3トップ画面](https://static.us-east-1.prod.workshops.aws/public/180149b9-f35e-46f7-93a7-bfdcb5fec962/static/07-RAG01.png)

2. 任意の名前を入力して、他の設定は編集せずに画面最下部の「バケットを作成」ボタンを押します。  
   （バケットの名前は全世界で一意である必要があります。）
    ![S3バケット作成](https://static.us-east-1.prod.workshops.aws/public/180149b9-f35e-46f7-93a7-bfdcb5fec962/static/07-RAG02.png)
    
3. バケットが作成されます。このバケットをナレッジベース作成のために使います。  
    ![S3バケット完成](https://static.us-east-1.prod.workshops.aws/public/180149b9-f35e-46f7-93a7-bfdcb5fec962/static/07-RAG03.png)
    
4. Amazon Bedrockのコンソールに移動し、左側のメニューの「構築」セクションの「ナレッジベース」をクリックします。  
...
7. データソースの設定画面に遷移します。ここでデータソースに使うS3バケットを選択します。「**S3 を参照**」ボタンを押します。  
    ![データソース設定](https://static.us-east-1.prod.workshops.aws/public/180149b9-f35e-46f7-93a7-bfdcb5fec962/static/07-RAG07.png)
    
8. S3バケットの一覧が表示されますので、先ほど作成したバケットを選択します。  
    ![S3バケット選択](https://static.us-east-1.prod.workshops.aws/public/180149b9-f35e-46f7-93a7-bfdcb5fec962/static/07-RAG08.png)
    
9. バケットがセットされたら「次へ」ボタンを押します。  
    ![データソース設定](https://static.us-east-1.prod.workshops.aws/public/180149b9-f35e-46f7-93a7-bfdcb5fec962/static/07-RAG09.png)
    

10. 埋め込みモデルとベクトルストアの設定画面に遷移します。「**埋め込みモデル**」ボタンを押して埋め込みモデル選択画面を開き、「`Titan Text Embeddings V2`」を選択します。  
![埋め込みモデル選択1](https://static.us-east-1.prod.workshops.aws/public/180149b9-f35e-46f7-93a7-bfdcb5fec962/static/07-RAG10a.png)

  
![埋め込みモデル選択2](https://static.us-east-1.prod.workshops.aws/public/180149b9-f35e-46f7-93a7-bfdcb5fec962/static/07-RAG10b.png)

11. ベクトルデータベースは「新しいベクトルストアをクイック作成」を選択し、「`Amazon OpenSearch Serverless`」が選択されていることを確認して「**次へ**」ボタンを押します。  
    ![ベクトルストアの選択](https://static.us-east-1.prod.workshops.aws/public/180149b9-f35e-46f7-93a7-bfdcb5fec962/static/07-RAG11.png)
    
12. 設定を確認して「**ナレッジベースを作成**」ボタンを押します。**ナレッジベースの作成中に他の画面に遷移すると作成プロセスが中断されてしまいますので、絶対に他の画面に遷移しないようにしましょう。**  
    ![ナレッジベースを作成](https://static.us-east-1.prod.workshops.aws/public/180149b9-f35e-46f7-93a7-bfdcb5fec962/static/07-RAG12.png)
    
13. ナレッジベースが作成できましたが、現在はファイルが存在しません。そこでAmazon S3バケットにファイルをアップロードします。S3コンソールでナレッジベースに紐づけたバケットを開き、「**アップロード**」ボタンを押します。  
    ![S3アップロード](https://static.us-east-1.prod.workshops.aws/public/180149b9-f35e-46f7-93a7-bfdcb5fec962/static/07-RAG14.png)
    

14. サンプルファイルとして、**架空のドラム式洗濯機のサービスマニュアル**を用意しました。ローカル環境にダウンロードした後ドラッグ＆ドロップして「**アップロード**」ボタンを押します。  
![S3アップロード](https://static.us-east-1.prod.workshops.aws/public/180149b9-f35e-46f7-93a7-bfdcb5fec962/static/07-RAG15.png)

15. アップロードが完了したら「**閉じる**」ボタンを押して終了します。  
    ![アップロード完了](https://static.us-east-1.prod.workshops.aws/public/180149b9-f35e-46f7-93a7-bfdcb5fec962/static/07-RAG16.png)
    
16. 先ほど作成したナレッジベースの画面に戻り、画面下部にある「データソース」セクションを見るとデータソースが一つ作られているのでラジオボタンをクリックしてチェックを入れて「**同期**」ボタンを押します。  
    ![データソース同期](https://static.us-east-1.prod.workshops.aws/public/180149b9-f35e-46f7-93a7-bfdcb5fec962/static/07-RAG17.png)
    
17. しばらく待つと同期が完了し、「最終同期時刻」が表示されたら成功です。 ![同期完了](https://static.us-east-1.prod.workshops.aws/public/180149b9-f35e-46f7-93a7-bfdcb5fec962/static/07-RAG18.png)
    

18. 作成したナレッジベースの「ナレッジベースID」をコピーしメモしておきます。

19. 続いて、ナレッジベースを参照するようLambdaのコードを以下のように編集しデプロイします。
...
20. この状態でサンプルアプリのWebページに戻り、ナレッジベースの内容を取得した上で応答できるか試してみましょう。
    
    例：「顧客から乾燥が不十分になってきたとのことで修理依頼を受けた。修理手順を説明して。」
    「DW-X9000の社内問い合わせ先はどこ？」「DW-X9000を修理する上で必要な工具を全て列挙して。」
# 1. はじめに
本ハンズオンで作成するアプリケーションのイメージは以下の通りです。
...
2. 画面の下方までスクロールし、**REST API を作成**をクリックします  
    ![REST APIクリック](https://static.us-east-1.prod.workshops.aws/public/180149b9-f35e-46f7-93a7-bfdcb5fec962/static/06-API02.png)
...
5. 画面下部に移動し、「ナレッジベース」メニューから「**作成**」ボタンを押してポップアップから「**ベクトルストアを含むナレッジベース**」を選択します。  
    ![ナレッジベースを作成](https://static.us-east-1.prod.workshops.aws/public/180149b9-f35e-46f7-93a7-bfdcb5fec962/static/07-RAG05.png)
